...
import rtx.vexsiko.api.modules.impl.Utils.guishare.GuiShareController;
...
import rtx.vexsiko.api.modules.impl.Utils.guishare.RemoteGuiPanelRenderer;
...
import rtx.vexsiko.stolen.noise.NoiseMaskPipeline;
...
public final class RemoteGuiWorld {
    ...
    public static void renderPanels(DrawContext drawContext) {
        ...
        // capture backdrop for noise on shattered panels (like screenshot, but with our noise)
        try {
            boolean anyShatter = false;
            for (RemoteGuiPanel p : list) if (p.shatterActive()) { anyShatter = true; break; }
            if (anyShatter) NoiseMaskPipeline.getInstance().captureBackdrop();
        ...
        try {
            for (int i = 0; i < n; ++i) {
                ...
                // noise overlay for shattered glass like screenshot (vexsiko noise)
                if (remoteGuiPanel.shatterActive()) {
                    try {
                        Identifier noiseTex = Identifier.of("vexsiko", "textures/noises/noise1.png");
                        float reveal = 1.0f - remoteGuiPanel.shatterProgressForRender();
                        reveal = Math.max(0f, Math.min(1f, reveal));
                        NoiseMaskPipeline.getInstance().draw(noiseTex, f6, f7, f8 * f4, f9 * f4, reveal, 3.0f, 0x101010, 0f, false, 0.006f);
                    } catch (Exception ignored) {}
                }
