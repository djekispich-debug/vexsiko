package rtx.vexsiko.utils.render.post.worldglow;
...
public final class WorldGlowRenderer {
    ...
    private static final Identifier PIPELINE_ID = Identifier.of("vexsiko", "pipeline/post/world_glow");
    private static final Identifier VERTEX_SHADER = Identifier.of("vexsiko", "post/world_glow/world_glow");
    private static final Identifier FRAGMENT_SHADER = Identifier.of("vexsiko", "post/world_glow/world_glow");
    ...
    public static void apply(Framebuffer framebuffer, int glowColor, float strength, float speed, float timeSeconds, Matrix4f projection, Matrix4f view, Vec3d cameraPos) {
        if (disabledAfterError || framebuffer == null) {
            return;
        }
        if (framebuffer.getColorAttachment() == null || framebuffer.getColorAttachmentView() == null || framebuffer.getDepthAttachment() == null) {
            return;
        }
        ...
        try {
            ...
            encoder.copyTextureToTexture(framebuffer.getDepthAttachment(), depthCopyTexture, 0, 0, 0, 0, 0, w, h);
        ...
        try {
            ...
            if (dummyVertexBuffer == null || dummyVertexBuffer.isClosed()) {
                ...
                dummyVertexBuffer = RenderSystem.getDevice().createBuffer(
                        () -> "vexsiko:world_glow_dummy_vertex",
    ...
    private static void writeUniforms(CommandEncoder encoder, int glowColor, float strength, float speed, float timeSeconds, Matrix4f projection, Matrix4f view, Vec3d cameraPos) {
