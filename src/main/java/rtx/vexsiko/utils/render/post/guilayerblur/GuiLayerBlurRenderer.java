package rtx.vexsiko.utils.render.post.guilayerblur;
...
import rtx.vexsiko.api.ui.window.GuiShatterAnimation;
import rtx.vexsiko.api.ui.window.GuiShatterAnimation.Remap;
import rtx.vexsiko.api.ui.window.WorldGuiCloseAnimation;
import rtx.vexsiko.mixin.accessor.GuiGraphicsExtractorAccessor;
import rtx.vexsiko.utils.render.others.RenderSampler;
...
public final class GuiLayerBlurRenderer {
    ...
    private static Identifier id(String string) {
        return Identifier.of((String)"vexsiko", (String)string);
    }
    ...
    private static boolean drawShards(CommandEncoder commandEncoder, GpuTextureView gpuTextureView, float f, float f2, float f3, boolean bl) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "vexsiko:gui_capture_shards", tempH.getColorAttachmentView(), OptionalInt.of(0));){
    ...
    public static void beginCapture(boolean bl, boolean bl2) {
        ...
        try {
            ...
            if (bl) {
                renderPass = commandEncoder.createRenderPass(() -> "vexsiko:gui_capture_clear", guiFbo.getColorAttachmentView(), OptionalInt.of(0), guiFbo.getDepthAttachmentView(), OptionalDouble.of(1.0));
                if (renderPass != null) {
                    renderPass.close();
                }
                captureActive = true;
            ...
            if (bl2 && GuiLayerBlurRenderer.ensureRemoteFbo(framebuffer.textureWidth, framebuffer.textureHeight)) {
                renderPass = commandEncoder.createRenderPass(() -> "vexsiko:gui_share_clear", remoteFbo.getColorAttachmentView(), OptionalInt.of(0), remoteFbo.getDepthAttachmentView(), OptionalDouble.of(1.0));
                if (renderPass != null) {
                    renderPass.close();
                }
    ...
    public static void composite(float f, float f2) {
        ...
        try {
            ...
            if (bl) {
                ...
                try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "vexsiko:gui_capture_shard_composite", framebuffer.getColorAttachmentView(), OptionalInt.empty());){
            ...
            if (bl2) {
                ...
                try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "vexsiko:gui_capture_composite_world", framebuffer.getColorAttachmentView(), OptionalInt.empty(), framebuffer.getDepthAttachmentView(), OptionalDouble.empty());){
            ...
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "vexsiko:gui_capture_composite", framebuffer.getColorAttachmentView(), OptionalInt.empty());){
                renderPass.setPipeline(compositePipeline);
                renderPass.bindTexture("uGui", gpuTextureView, RenderSampler.linear());
                renderPass.setUniform("CompositeData", compositeUniform);
                renderPass.draw(0, 6);
        ...
        block6: {
            ...
            try {
                ...
                if (worldDepthCopy == null) {
                    ...
                    worldDepthCopy = gpuDevice.createTexture(() -> "vexsiko:gui_share_world_depth", 5, TextureFormat.DEPTH32, framebuffer.textureWidth, framebuffer.textureHeight, 1, 1);
                ...
                VexSiko.LOGGER.warn("[GuiShare] world depth snapshot failed", throwable);
            }
        }
    }
        ...
        block6: {
            ...
            try {
                ...
                if (handDepthCopy == null) {
                    GpuDevice gpuDevice = RenderSystem.getDevice();
                    handDepthCopy = gpuDevice.createTexture(() -> "vexsiko:gui_share_hand_depth", 5, TextureFormat.DEPTH32, framebuffer.textureWidth, framebuffer.textureHeight, 1, 1);
                ...
                VexSiko.LOGGER.warn("[GuiShare] hand depth snapshot failed", throwable);
            }
        }
    }
    ...
    public static Framebuffer worldBackdropSource(Framebuffer framebuffer) {
        ...
        try {
            ...
            if (list.isEmpty()) {
                ...
                try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "vexsiko:world_backdrop_warp", sceneSnapshot.getColorAttachmentView(), OptionalInt.of(0));){
            ...
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "vexsiko:world_backdrop_warp_slots", sceneSnapshot.getColorAttachmentView(), OptionalInt.of(0));){
    ...
    public static Framebuffer worldSnapshotWithPanels() {
        ...
        try {
            ...
            CommandEncoder commandEncoder = RenderSystem.getDevice().createCommandEncoder();
            GuiLayerBlurRenderer.writeCompositeUniform(commandEncoder, 1.0f, 1.0f);
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "vexsiko:gui_share_popup_backdrop", sceneSnapshot.getColorAttachmentView(), OptionalInt.empty());){
                renderPass.setPipeline(compositePipeline);
    ...
    public static void markPanelEnd(DrawContext drawContext) {
        ...
        GuiRenderState guiRenderState = ((GuiGraphicsExtractorAccessor)drawContext).vexsiko_getGuiRenderState();
    ...
    private static void ensureSceneSnapshotTarget(int n, int n2) {
        ...
        sceneSnapshot = GuiLayerBlurRenderer.destroy(sceneSnapshot);
        try {
            sceneSnapshot = new SimpleFramebuffer("vexsiko_world_backdrop_snapshot", n, n2, false);
        }
        catch (Throwable throwable) {
            sceneSnapshot = null;
        }
    ...
    private static void gaussianPass(CommandEncoder commandEncoder, GpuTextureView gpuTextureView, SimpleFramebuffer simpleFramebuffer, float f, float f2) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "vexsiko:gui_capture_gaussian", simpleFramebuffer.getColorAttachmentView(), OptionalInt.of(0))) {
    ...
    private static void blitPanelsIntoPopupSlots(CommandEncoder commandEncoder) {
        ...
        for (RemoteGuiWorld.PanelQuad panelQuad : list) {
            ...
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "vexsiko:gui_share_popup_slot_blit", sceneSnapshot.getColorAttachmentView(), OptionalInt.empty())) {
    ...
    private static boolean ensureTargets(int n, int n2) {
        ...
        GuiLayerBlurRenderer.closeTargets();
        guiFbo = new SimpleFramebuffer("vexsiko_gui_capture", n, n2, true);
        tempH = new SimpleFramebuffer("vexsiko_gui_capture_h", n, n2, false);
        tempV = new SimpleFramebuffer("vexsiko_gui_capture_v", n, n2, false);
        texWidth = n;
        texHeight = n2;
        return true;
    }
    ...
    private static void capturePreCompositeColor(Framebuffer framebuffer) {
        ...
        try {
            ...
            if (preCompositeColor == null) {
                GpuDevice gpuDevice = RenderSystem.getDevice();
                preCompositeColor = gpuDevice.createTexture(() -> "vexsiko:gui_share_scene_before", 5, TextureFormat.RGBA8, framebuffer.textureWidth, framebuffer.textureHeight, 1, 1);
    ...
    private static boolean ensureRemoteFbo(int n, int n2) {
        ...
        return (remoteFbo = new SimpleFramebuffer("vexsiko_gui_share_capture", n, n2, true)).getColorAttachmentView() != null;
    }
    ...
    private static void drawRemoteQuad(CommandEncoder commandEncoder, Framebuffer framebuffer, boolean bl) {
        if (bl) {
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "vexsiko:gui_share_composite", framebuffer.getColorAttachmentView(), OptionalInt.empty());){
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "vexsiko:gui_share_composite", framebuffer.getColorAttachmentView(), OptionalInt.empty(), framebuffer.getDepthAttachmentView(), OptionalDouble.empty());){
            renderPass.setPipeline(worldCompositePipeline);
            renderPass.bindTexture("uGui", remoteFbo.getColorAttachmentView(), RenderSampler.linear());
            renderPass.setUniform("CompositeData", compositeUniform);
            renderPass.setUniform("WorldQuadData", worldQuadUniform);
    ...
    private static boolean ensurePipelines() {
        ...
        if (shardVertexBuffer == null) {
            shardVertexBuffer = RenderSystem.getDevice().createBuffer(() -> "vexsiko:guilayerblur_shard_vertices", 40, 44352L);
        }
        if (shardVertexData == null) {
            shardVertexData = ByteBuffer.allocateDirect(44352).order(ByteOrder.nativeOrder());
        }
        if (blurUniform == null) {
            blurUniform = RenderSystem.getDevice().createBuffer(() -> "vexsiko:guilayerblur_blur_uniform", 136, 16L);
        }
        if (compositeUniform == null) {
            compositeUniform = RenderSystem.getDevice().createBuffer(() -> "vexsiko:guilayerblur_composite_uniform", 136, 16L);
        }
        if (worldQuadUniform == null) {
            worldQuadUniform = RenderSystem.getDevice().createBuffer(() -> "vexsiko:guilayerblur_world_quad_uniform", 136, 96L);
        }
        if (worldSlotsUniform == null) {
            worldSlotsUniform = RenderSystem.getDevice().createBuffer(() -> "vexsiko:guilayerblur_world_warp_uniform", 136, 512L);
        }
        if (slotBlitUniform == null) {
            slotBlitUniform = RenderSystem.getDevice().createBuffer(() -> "vexsiko:guilayerblur_slot_blit_uniform", 136, 64L);
        }
    ...
    private static boolean drawRemoteShards(CommandEncoder commandEncoder, Framebuffer framebuffer, RemoteGuiWorld.PanelQuad panelQuad, Matrix4f matrix4f) {
        ...
        int n = ((rtx.vexsiko.api.ui.window.GuiShatterAnimation.State)shatter.state()).buildGeometry(shardVertexData, shatter.progress(), shatter.alpha(), 1.0f, true, remap);
        if (n <= 0 || shardVertexData.remaining() <= 0) {
            return false;
        }
        ...
        commandEncoder.writeToBuffer(shardVertexBuffer.slice(0L, (long)shardVertexData.remaining()), shardVertexData);
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "vexsiko:gui_share_shards", framebuffer.getColorAttachmentView(), OptionalInt.empty());){
    ...
    private static void disableAfterError(Throwable throwable) {
        ...
        captureActive = false;
        GuiLayerBlurRenderer.closeTargets();
        VexSiko.LOGGER.error("[GuiLayerBlur] disabled after error", throwable);
    }
