package rtx.vexsiko.utils.render.render2d.sectormask;
...
import rtx.vexsiko.utils.render.render2d.sectormask.BuiltSectorMask;
import rtx.vexsiko.utils.render.render2d.sectormask.SectorMaskRenderState;
...
implements AutoCloseable {
    ...
    public void submit(DrawContext drawContext, BuiltSectorMask builtSectorMask) {
        ...
        try {
            Matrix3x2f matrix3x2f = rtx.vexsiko.utils.render.render2d.Render2DCoordinateSpace.pose(drawContext);
            ((rtx.vexsiko.mixin.accessor.GuiGraphicsExtractorAccessor)drawContext).vexsiko_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new SectorMaskRenderState(matrix3x2f, builtSectorMask, rtx.vexsiko.utils.render.scissor.ScissorUtil.current()));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "vexsiko_sector_mask_params", 136, 640L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedMasks);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "vexsiko_sector_mask_params", 128, byteBuffer);
            this.paramsDirty = false;
