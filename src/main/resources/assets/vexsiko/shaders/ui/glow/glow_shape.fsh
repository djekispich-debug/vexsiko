...
#moj_import <vexsiko:releon_common.glsl>
...
vec2 vexsikoFrag = vec2(0.0);

vec3 glowPaletteColorAt(int idx) {
    if (params[78].w > 0.5) {
        return vexsikoClientStop(idx, vexsikoFrag);
    }
...
void main() {
    ...
    vexsikoFrag = vexsikoFragXYMappedFlipY(pixel, params[78]);
